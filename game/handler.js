...
class Handler{
    ...
    handleQuickPlay(client, packet){
        var mapDetails = this.WOL.validateMapDetails(client, packet);
        if(DEBUG)
            console.log(">> Validated map details");
        var gameId = this.WOL.findSlot(client, mapDetails, null);
            ...
            if(n > 0){
                client.db.fetch({"dname" : data.userId, "gkey" : client.gameSession}, (function(doc){
                    ...
                    if(client.getGame().isFull()){
                        client.gameId = this.WOL.findSimilarSlot(client.getGame());
                    }
    ...
    handleProjectile(client, data){
        ...
        var properties = this.WOL.weaponsObj[data.ammo_type];
       ...
       */
        if(properties.timeAfter == null){ //harmful
            client.avatar.alreadyShot = true;
            if(DEBUG){
                console.log("Setting next turn at default time after weapon delay: " + this.WOL.DEFAULT_TIME_AFTER_WEAPON);
            }
            client.getGame().setNextTurnFN(this.WOL.DEFAULT_TIME_AFTER_WEAPON);
    ...
    handleBuyAccessory(client, data){
        ...
        if(!this.WOL.accessoriesObj[type]){
            console.log("type of accessory not found!");
            return;
        }

        if(this.WOL.accessoriesObj[type].currency == "treats"){
            if(client.chargeTreats(this.WOL.accessoriesObj[type].price)){
            ...
            if(client.chargeGold(this.WOL.accessoriesObj[type].price)){
                client.player.userAccessories.push(type);
                client.sendUpdate();
                client.updatePlayerData();
            }else{
    ...
    handleAccLoad(client, data){
        ...
        for(var key in data.load){
            ...
            occurrences[this.WOL.accessoriesObj[item].type] = (occurrences[this.WOL.accessoriesObj[item].type] != null) ? occurrences[this.WOL.accessoriesObj[item].type] + 1 : 1;
            if(occurrences[this.WOL.accessoriesObj[item].type] > 1){
                valid = false;
                break;
            }
        }
    ...
    handleBuyPet(client, data){
        ...
        if(!data.name) return;
         ...
         if(this.WOL.config.petDetailColors.indexOf(data.color2.toString(16)) < 0 || this.WOL.config.petMainColors.indexOf(data.color1.toString(16)) < 0){
            if(DEBUG) console.log(">>> Color hack!");
            return;
        }
        
        if(!this.WOL.petsObj[data.type]){
            if(DEBUG) console.log(">>> pet does not exist in crumbs " + data.type);
            return;
        }

        if(this.WOL.petsObj[data.type].currency == "treats"){
            if(!client.chargeTreats(parseInt(this.WOL.petsObj[data.type].price))){
                if(DEBUG) console.log(">> not enough treats for pet!");
                return;
            }
        }else{
            if(!client.chargeGold(parseInt(this.WOL.petsObj[data.type].price))){
    ...
    handleBuyAmmo(client, data){
        ...
        if(this.WOL.weaponsObj[type].currency == "treats"){
            if(client.chargeTreats(this.WOL.weaponsObj[type].price * data.ammoCount)){
                if(client.player.userWeaponsOwned[type]) client.player.userWeaponsOwned[type] += (this.WOL.weaponsObj[type].purchaseAmount ? (parseInt(this.WOL.weaponsObj[type].purchaseAmount) * data.ammoCount) : data.ammoCount);
                else client.player.userWeaponsOwned[type] = (this.WOL.weaponsObj[type].purchaseAmount ? (parseInt(this.WOL.weaponsObj[type].purchaseAmount) * data.ammoCount) : data.ammoCount);
            ...
            if(client.chargeGold(this.WOL.weaponsObj[type].price * data.ammoCount)){
                if(client.player.userWeaponsOwned[type]) client.player.userWeaponsOwned[type] += (this.WOL.weaponsObj[type].purchaseAmount ? (parseInt(this.WOL.weaponsObj[type].purchaseAmount) * data.ammoCount) : data.ammoCount);
                else client.player.userWeaponsOwned[type] = (this.WOL.weaponsObj[type].purchaseAmount ? (parseInt(this.WOL.weaponsObj[type].purchaseAmount) * data.ammoCount) : data.ammoCount);
                client.sendUpdate();
                client.updatePlayerData();
    ...
    handleCreateGame(client, data){
        ...
        if(this.WOL.getGame(data.gameName)){
            client.sendPacket({
                command:"game_name_return",
                name: data.gameName,
                value: 0
        ...
        var mapDetails = this.WOL.validateMapDetails(client, data);
        var gameId     = this.WOL.findSlot(client, mapDetails, data.gameName);
    ...
    handleJoinGame(client, data){
        ...
        data.gameName = data.gameName.split(' ').join('-');
        var gameRef = this.WOL.getGame(data.gameName);
        if(gameRef){
